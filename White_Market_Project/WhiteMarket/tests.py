...
class RegistrationTests(TestCase):
    ...
    def test_good_form_creation(self, validate_method):
        ...
        profile_data = {'profilePicture': tempfile.NamedTemporaryFile(suffix=".jpg").name, 'description': "I love baseball cards!", 'phoneNo':'07225652981', "captcha": "XXX",}
    ...
    @patch("captcha.fields.ReCaptchaField.validate")
    def test_good_registration_post_response(self, validate_method):
        """
        Checks the POST response of the registration view.
        We should be able to log a user in with new details after this!
        """
        post_data = {'username': 'qwerty', 'password': 'test123', 'email': 'test@test.com', 'profilePicture': tempfile.NamedTemporaryFile(suffix=".jpg").name, 'description': "I love baseball cards!", 'phoneNo':'07225652981', "captcha": "XXX",}
        validate_method.return_value = True
...
class LoginTests(TestCase):
    @patch("captcha.fields.ReCaptchaField.validate")
    def test_login_functionality(self, validate_method):
        """
        Tests the login functionality. A user should be able to log in.
        """
        user_object = create_user_object()
        validate_method.return_value = True
        response = self.client.post(reverse('whitemarket:login'), {'username': 'testuser', 'password': 'testabc123', "captcha": "XXX"})
...
class SellerTests(TestCase):
    ...
    def test_list_item_form(self):
        ...
        item_data = {'itemName': 'Babe Ruth baseball card', 'isDigital': False, 'itemDescription': 'Famous baseball card',
                    'storeID': item_store.storeID, 'condition': CONDITION_CHOICES[1][0],  'buyNowPrice':100,}
